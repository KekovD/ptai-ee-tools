package com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v490;
...
import static com.ptsecurity.appsec.ai.ee.scan.result.ScanBrief.ApiVersion.V490;
...
@Slf4j
@DisplayName("Test PT AI 4.9.0 REST API data structures")
@Tag("development")
@Environment(enabledFor = { V490 })
public class RestApiDataStructuresIT extends BaseClientIT {
    ...
    protected void generateData(@NonNull final Path destination, @NonNull final ProjectTemplate.ID templateId, @NonNull final Consumer<UnifiedAiProjScanSettings> modifySettings) {
        ...
        com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v490.ApiClient client = new ApiClient(connectionSettings);
        // Initialize all API clients with URL, timeouts, SSL settings etc.
        client.init();
        client.authenticate();

        Path jsons = destination.resolve("v490").resolve("json");
        jsons.toFile().mkdirs();
